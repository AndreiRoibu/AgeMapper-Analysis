...
function [icaS,icaA,pcaU,pcaS,pcaV,correlations_dict] = ICAdim2(X,Jvalues,C,RepeatsPerJ,itr);
...
% itr = 3000;         % number of iterations. default = 3000
...
for J=Jvalues
  ...
  for JJ=1:RepeatsPerJ
    ...
    [icaS,icaA,~]   = fastica(pcaV(:,1:J)', 'approach',aprc,'g',g,'epsilon',epsilon,'maxNumIterations',itr, 'verbose', vrb,'lastEig',J);
    [icaS1,icaA1,~] = fastica(pcaV1',       'approach',aprc,'g',g,'epsilon',epsilon,'maxNumIterations',itr, 'verbose', vrb,'lastEig',J);
    [icaS2,icaA2,~] = fastica(pcaV2',       'approach',aprc,'g',g,'epsilon',epsilon,'maxNumIterations',itr, 'verbose', vrb,'lastEig',J);
    if C==1
...
if length(Jvalues)>1
  figure; plot(Jvalues,NStrongCorrs); title('Number of Strong Correlations'); drawnow;
  figure; plot(Jvalues,NStrongCorrs_sum); title('Sum of Strong Correlations');drawnow;
end
NStrongCorrs
